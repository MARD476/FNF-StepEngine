...
class SMFile
{
	...
	public function new(data:Array<String>)
	{
		try
		{
			...
			while (inc < data.length)
			{
				...
				if (StringTools.startsWith(line.trim(), "#NOTES:"))
					break;
			...
			var musicFileLower:String = header.MUSIC.toLowerCase();
			if (!StringTools.endsWith(musicFileLower, "ogg") && !StringTools.endsWith(musicFileLower, "mp3"))
			{
				Application.current.window.alert("The music MUST be an OGG or MP3 File, make sure the sm file has the right music property.",
	...
	public function convertToFNF(saveTo:String):String
	{
		...
		for (measure in measures)
		{
			...
			for (i in 0...measure._measure.length - 1)
			{
				...
				for (i in notes)
				{
					...
					if (i == "M")
					{
						var mineData:Array<Dynamic> = [rowTime, lane, 0, "Hurt Note", 0];
                        section.sectionNotes.push(mineData);
						index++;
						continue;
					}
		...
		{
			// Creamos un nuevo array temporal que sigue la estructura de Psych Engine
			var convertedEvents:Array<Array<Dynamic>> = [];
			var convertedEvents2:Array<Array<Dynamic>> = [];
			
			for (change in header.changeEvents)
			{
				// PARA LOS EVENTS VE
				...
				var bpm_original:Float = header.getBPM(0);
				var seg = TimingStruct.getTimingAtBeat(currentBeat);

				convertedEvents.push(event);
			}
			
			song.events = convertedEvents;
			song.events = convertedEvents2;
		}

		for (stop in header.stopEvents)
		{
			song.events.push([stop[0], [["Stop Scroll", Std.string(stop[1]), ""]]]);
		}

		for (speed in header.speedEvents)
		{
			song.events.push([speed[0], [["Change Scroll Speed Step", Std.string(speed[1]), ""]]]);
		}

		for (bg in header.bgChangeEvents)
		{
			song.events.push([bg[0], [["BG Change", bg[1], ""]]]);
		}
